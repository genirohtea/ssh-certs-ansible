...
The role has four sub-tasks. Each is opt-in via a boolean variable, so the
same role works on both ends of the trust relationship:

- `sign_host_key` — sign the server's host key with the host CA, install
  `HostCertificate` in `/etc/ssh/sshd_config`, and restart sshd.
- `accept_user_ca` — install the user CA public key on a server and add
  `TrustedUserCAKeys` to `/etc/ssh/sshd_config`, so the server accepts
  user-cert-based logins.
- `sign_user_key` — generate (if missing) an Ed25519 key on a workstation
  and sign it with the user CA, producing `~/.ssh/<name>-cert.pub`.
- `accept_host_ca` — add a `@cert-authority` line to the workstation's
  `~/.ssh/known_hosts` and append a `Host` stanza to `~/.ssh/config`.

Requirements
------------

Ansible 2.14+ on the control node and the following collections (declared
...
No role dependencies. Collection dependencies are declared in
`meta/main.yml` and `requirements.yml`.
